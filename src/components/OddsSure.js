...
class OddsSure extends Component {
  ...
  bindList() {
    ...
    common.getData('data/odds.php?data=sure&date=' + this.state.filter_date + '&user_id=' + userId).then((items) => {

      items.forEach(x => {
        if (Number(x.total) < 100) {
          let odd365 = Number(x.odd_365);
          let oddEsp = Number(x.odd_espnet);
          let oddSmall = odd365 < oddEsp ? odd365 : oddEsp;
          let oddBig = odd365 > oddEsp ? odd365 : oddEsp;
            ...
            <table id="table-odds-sure" className="table-odds-sure table-scroll table-odds w-100" onClick={this.checkHideFilter} >
              <thead  >
                <tr>
                  ...
                  <th onClick={common.tableSortNumber.bind(this, 'total')} >Bet</th>
                </tr>
              </thead>
              <tbody>
...
export default withRouter(OddsSure);
